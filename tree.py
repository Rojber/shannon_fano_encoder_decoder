class Tree:
    ...
    def get_codes_dict(self, codes_dict, path):
        if self.left is not None:
            if self.left.data != '_NewLine' and self.left.data != '_Space' and len(self.left.data) > 1:
                self.left.get_codes_dict(codes_dict, path + [0])
            else:
                codes_dict[self.left.data] = ''.join(str(x) for x in path + [0])
        if self.right is not None:
            if self.right.data != '_NewLine' and self.right.data != '_Space' and len(self.right.data) > 1:
